...
void listarClientes()
{
    ...
    do
    {
        ...
        escolha = verificacao('2', "\n  " RED "1 -" RESET " Detalhar Pedido" "\n  " RED "0 -" RESET " Sair" "\n\nInsira uma opção: ", RED "ERRO! Digite uma opção válida" RESET);

        int codigoCliente;

        if(escolha == '1')
        {
            codigoCliente = verificacaoCodigo("\n\n  Insira o ID do cliente para detalhar pedidos(0 - Voltar): ", RED "  ERRO! Digite um ID de cliente válido" RESET);
            system("cls");
        }
